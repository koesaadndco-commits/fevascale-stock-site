...
const REGISTRY = {
  ...
  bulletin_posts:    { model: 'bulletinPost',   pk: 'id' },
  app_settings:      { model: 'appSetting',     pk: 'key' },
  app_config:        { model: 'appConfig',      pk: 'key' },
  profiles:          { model: 'profile',        pk: 'id' },
};

// ログイン前でも読める（＝セッション不要）テーブル。ログイン画面のブランディング用。
// app_config は AI APIキー等の機密を含むため公開読み取りにしない。
export const PUBLIC_READ_TABLES = new Set(['app_settings']);
// 書き込みに管理者権限を要するテーブル（認証情報・全体設定）
const ADMIN_WRITE_TABLES = new Set(['app_users', 'app_settings', 'app_config', 'profiles']);
// 非管理者には値を返さない app_config のキー（機密）
const SECRET_CONFIG_KEYS = new Set(['ai_api_key']);
...
function stripSecret(table, rows, isAdmin) {
  // app_users の password は誰にも返さない
  if (table === 'app_users') {
  ...
  // app_config の機密キーは管理者以外に値を返さない
  if (table === 'app_config' && !isAdmin) {
    const clean = (o) => (o && typeof o === 'object' && SECRET_CONFIG_KEYS.has(o.key)) ? { ...o, value: '' } : o;
    return Array.isArray(rows) ? rows.map(clean) : clean(rows);
  }
  return rows;
}
...
export async function executeQuery(spec, user) {
  ...
  try {
    ...
    if (action === 'select') {
      ...
      // 件数取得（.select(..., { count, head }))
      if (spec.count) {
        const count = await d.count({ where });
        if (spec.head) return { data: null, count, error: null };
        const rows = await d.findMany(args);
        return { data: stripSecret(table, rows, isAdmin), count, error: null };
      }
      if (spec.single === 'maybe') {
        const row = await d.findFirst(args);
        return { data: stripSecret(table, row, isAdmin) || null, error: null };
      }
      const rows = await d.findMany(args);
      return { data: stripSecret(table, rows, isAdmin), error: null };
    }

    if (action === 'insert') {
      const rows = (spec.rows || []).map((row) => sanitizeWrite(table, row));
      await d.createMany({ data: rows, skipDuplicates: false });
      return { data: spec.returning ? stripSecret(table, rows, isAdmin) : null, error: null };
    }
    ...
    if (action === 'upsert') {
      ...
      return { data: spec.returning ? stripSecret(table, out, isAdmin) : null, error: null };
    }
...
// 空のパスワードは「変更なし」を意味するのでキーごと除去する
// （APIの返却時に password を空にしているため、そのまま保存すると
//   既存ハッシュが空文字で上書きされ全員ログイン不能になる）。
function sanitizeWrite(table, row) {
  const out = { ...row };
  if (table === 'app_users') {
    if (typeof out.password !== 'string' || out.password === '') {
      delete out.password;
    } else if (!isHashed(out.password)) {
